...
STRAT_ORDER  = ["Trend", "MeanReversion", "Defensive"]
STRAT_LABELS = {"Trend": "추세추종", "MeanReversion": "평균회귀", "Defensive": "방어적"}
...
rf_daily = 0.03 / 252
snap["roll_sharpe"] = (
    (dr - rf_daily).rolling(30).mean()
    / (dr - rf_daily).rolling(30).std()
    * np.sqrt(252)
)

# 전략 숫자 인코딩 (시계열 그래프용)
strat_map = {s: i for i, s in enumerate(STRAT_ORDER)}
snap["strat_num"] = snap["strategy"].map(strat_map)

# 전략별 30일 롤링 비율
for s in STRAT_ORDER:
    snap[f"roll_{s}"] = (snap["strategy"] == s).rolling(30).mean() * 100

# ── 레이아웃: 6행 2열 ─────────────────────────────────────────
fig = plt.figure(figsize=(18, 26), facecolor=COLORS["bg"])
gs  = GridSpec(6, 2, figure=fig,
               hspace=0.50, wspace=0.30,
               left=0.07, right=0.96, top=0.94, bottom=0.04)

ax_cum      = fig.add_subplot(gs[0, :])   # 누적 수익률
ax_cash     = fig.add_subplot(gs[1, 0])   # 현금 비율
ax_pos      = fig.add_subplot(gs[1, 1])   # 포지션 수
ax_mdd      = fig.add_subplot(gs[2, :])   # MDD
ax_strat_ts = fig.add_subplot(gs[3, :])   # ★ 전략 시계열 (NEW, 전체 너비)
ax_sharpe   = fig.add_subplot(gs[4, 0])   # 롤링 샤프
ax_strat_pi = fig.add_subplot(gs[4, 1])   # 전략 분포 파이
ax_ret      = fig.add_subplot(gs[5, 0])   # 거래 수익률 분포
ax_metric   = fig.add_subplot(gs[5, 1])   # 핵심 지표 표

all_axes = [ax_cum, ax_cash, ax_pos, ax_mdd, ax_strat_ts,
            ax_sharpe, ax_strat_pi, ax_ret, ax_metric]
for ax in all_axes:
...
ax.plot(snap.index, snap["cum_pct"], color=COLORS["portfolio"], lw=1.5)
...
c_ret = COLORS["profit"] if final_ret >= 0 else COLORS["loss"]
ax.annotate(f"최종: {final_ret:+.2f}%",
            xy=(snap.index[-1], final_ret),
            xytext=(-60, 10), textcoords="offset points",
            color=c_ret, fontsize=10, fontweight="bold",
            arrowprops=dict(arrowstyle="->", color=c_ret, lw=1.2))

# ── 2. 현금 비율 ──────────────────────────────────────────────
ax = ax_cash
ax.fill_between(snap.index, snap["cash_pct"], color=COLORS["cash"], alpha=0.5)
ax.plot(snap.index, snap["cash_pct"], color=COLORS["cash"], lw=1.2)
ax.axhline(10, color="#FFD54F", lw=0.8, ls=":", alpha=0.8)
ax.text(snap.index[5], 11.5, "최소 현금 10%", color="#FFD54F", fontsize=7)
...
ax.plot(snap.index, snap["drawdown"], color=COLORS["mdd"], lw=1.3)
...
# 위쪽: 전략 전환 산점도 (언제 어떤 전략이었는지 점으로 표시)
for i, s in enumerate(STRAT_ORDER):
    mask = snap["strategy"] == s
    ax.scatter(snap.index[mask],
               np.full(mask.sum(), i),
               c=COLORS[s], s=6, alpha=0.6,
               label=f"{STRAT_LABELS[s]} ({mask.sum()}일)", zorder=3)

# 전략 전환 시점 수직선
prev = snap["strat_num"].iloc[0]
switch_dates = []
for idx, val in snap["strat_num"].items():
    if val != prev:
        switch_dates.append(idx)
        prev = val

for d in switch_dates:
    ax.axvline(d, color="#444", lw=0.5, alpha=0.5)

# y축 레이블을 전략명으로
ax.set_yticks(range(len(STRAT_ORDER)))
ax.set_yticklabels(
    [f"{STRAT_LABELS[s]}" for s in STRAT_ORDER],
...
for i, s in enumerate(STRAT_ORDER):
    ax.axhspan(i - 0.4, i + 0.4,
               color=COLORS[s], alpha=0.07)

ax.set_title(
    f"전략 시계열  |  전체 전환 횟수: {len(switch_dates)}회",
    color=COLORS["text"], fontsize=12, pad=8
)
ax.grid(True, color=COLORS["grid"], lw=0.5, axis="x")
style_xaxis(ax)
add_split_lines(ax)
...
wedges, texts, autotexts = ax.pie(
    strat_counts.values,
    labels=[STRAT_LABELS.get(s, s) for s in strat_counts.index],
    colors=strat_colors,
    autopct="%1.1f%%",
    startangle=140,
...
def fmt(k, v):
    try:
        f = float(v)
        if any(x in k for x in ["수익률", "MDD", "승률", "수익"]):
            return f"{f:+.2f}%"
        if "Sharpe" in k:
            return f"{f:.4f}"
        if "자산" in k:
...
rows = [[k, fmt(k, metrics[k])] for k in display_keys if k in metrics]
if not rows:
    rows = [
...
plt.savefig(OUTPUT, dpi=150, bbox_inches="tight", facecolor=COLORS["bg"])
